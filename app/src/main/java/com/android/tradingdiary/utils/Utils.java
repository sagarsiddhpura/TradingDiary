...
public class Utils {
    ...
    public static void setupActionBarResolvedColor(Activity activity, boolean isLightBar, int bgColorRes,
                                      int bgColorRes2, int titleRes, Toolbar toolbar) {
        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            window.setStatusBarColor(bgColorRes2);
            if (!isLightBar) {
                toolbar.setTitleTextColor(activity.getResources().getColor(R.color.white));
            }
        } else {
            window.setStatusBarColor(bgColorRes);
            if (isLightBar) {
                window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
            } else {
                toolbar.setTitleTextColor(activity.getResources().getColor(R.color.white));
